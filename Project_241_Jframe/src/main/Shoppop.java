...
public class Shoppop extends MyArrayList{
	...
	public void showItem() throws Exception {
		AllItemList = "";
	     ...
	     AllItemList +="Number : " + ++number + "<br>"+"Item: "+ allItem.get(i).getName()  + "<br>"+"Price: "+ allItem.get(i).getPrice()  + "<br>"+"Amount: "+ allItem.get(i).getAmount()  
	    		 + "<br>"+"---------------------------------------------------------------------------------"+"<br>";
	   ...
	   public void addItem() {
	       ...
	       if(name.matches("[^a-zA-Z]") || name.matches("") || !price.matches("[0-9]+") || price.matches("")|| !amount.matches("[0-9]+") || amount.matches(""))  {
	   ...
	   public void removeByNumber() {
		 ...
		 public void removeByName() {
			     ...
			     if(founded){
			         //System.out.println(allItem.get(x).getName() + "\s" +"\s"+"\s" +"\s" + allItem.get(x).getPrice()+ "\s" +"\s"+"\s" +"\s" +allItem.get(x).getAmount());
			     	allItem.remove(y);
			     }
			     else{
	   ...
	   public void search(){
	        ...
	        if(found){
	            //System.out.println(allItem.get(x).getName() + "\s" +"\s"+"\s" +"\s" + allItem.get(x).getPrice()+ "\s" +"\s"+"\s" +"\s" +allItem.get(x).getAmount());
	        	JOptionPane.showMessageDialog(null, "Item : " + allItem.get(x).getName() + "\s" +"\s"+"\s" +"\s" + "Price : " + allItem.get(x).getPrice()+ "\s" +"\s"+"\s" +"\s" +"Amount : " + allItem.get(x).getAmount());
	        }
	        else{
	           // System.out.print(searchItem+" is not found");
